...
const DishFormModal = ({ isOpen, onClose }) => {
  ...
  const handleAddItem = () => {
    const itemToAdd = items.find(item => 
      item.id === selectedItem || item.name === selectedItem
    );
    
    if (itemToAdd) {
      setSelectedItems(prev => [...prev, {
        name: itemToAdd.name,
        calories: itemToAdd.calories || "0 Kcal",
        quantity: quantity
      }]);
        ...
        <div className="space-y-4">
          ...
          <div className="mx-auto w-3/4">
            <input
              ...
              className="w-full px-4 py-2 rounded-xl shadow-[0_4px_4px_0_#0000001f,_inset_1px_0_4px_rgba(0,0,0,0.11)] border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#FADA90]"
            />
          </div>
          ...
          <div className="space-y-3 m-auto w-3/4">
            ...
            <div className="text-center">
              <button 
                ...
                className="py-1.5 px-8 bg-[#FADA90] rounded-full hover:bg-[#f5d4b3] transition-colors text-sm"
              >
                Add Items
              </button>
            </div>
